...
describe('basic test', () => {
  ...
  it('none number', (done) => {
   ...
   it('available nedpoints', (done) => {
    req.get(`${API_URL}/available_payments`, (_err, res, body) => {
      chai.expect(res.statusCode).to.be.equal(200);
   ...
   it('login', (done) => {
    req.post(`${API_URL}/login`, { json: { userName: 'Pinky' } }, (_err, res, body) => {
      chai.expect(res.statusCode).to.be.equal(200);
      chai.expect(body).to.be.equal('Welcome Pinky');
      done();
    });
  });
});
